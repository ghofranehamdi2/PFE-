...
- **Modèle principal :** YOLOv26 (poids fournis : [yolo26n.pt](yolo26n.pt)).
...
-------------------------
- **Type :** Détection d'objets en temps réel (You Only Look Once style) — YOLOv26n (nano) pour faible latence.
- **Poids fournis :** `yolo26n.pt` (quantité, taille et date dans le dépôt).
- **Pipeline général :** acquisition -> prétraitement -> inférence (YOLOv26) -> post-traitement -> calcul du `vision_score` -> envoi au backend.
...
--------------------------
- **Entrées attendues :** image RGB (BGR selon OpenCV conversion), résolution recommandée 640×640 pour YOLOv26n.
...
-----------------------------
- **Sorties typiques YOLOv26 :** pour chaque détection → `[x1, y1, x2, y2, confidence, class_id]`.
...
- $C$ : la `confidence` fournie par YOLOv26.
...
--------------------------
- **Local / edge (Raspberry Pi) :** utiliser version légère (yolo26n), GPU si disponible (NVIDIA Jetson ou inference accélérée), sinon CPU en mode optimisé.
...
```bash
python pi_client/main_cv.py --source 0 --weights yolo26n.pt --conf 0.25
...
-------------------------------
- Poids modèle : [yolo26n.pt](yolo26n.pt)
